...
// 필터는 모델 그대로 두 축이다. 구독 축의 칩은 합이 전체와 같고(탈퇴 포함), 알림 축의 칩은
// 선택된 구독 안에서 센다 — 이용 중을 고르면 켜짐·꺼짐·일시중지가 개요 카드의 수와 같다.
type Filter = 'all' | SubscriptionStateKey;
type AlertFilter = 'all' | 'on' | 'off' | 'paused';
...
const ALERT_FILTERS: { id: AlertFilter; label: string }[] = [
  { id: 'all', label: '전체' },
  { id: 'on', label: '켜짐' },
  { id: 'off', label: '꺼짐' },
...
function alertKey(summary: { key: string }): Exclude<AlertFilter, 'all'> {
  if (summary.key === 'paused') return 'paused';
  if (summary.key === 'off') return 'off';
  return 'on';
}
...
function FilterRow<T extends string>({
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <span className="w-8 text-xs text-gray-400 dark:text-gray-500">{label}</span>
      {items.map((f) => {
...
export default function UsersTab({
  ...
  const seats = rows.filter((r) => r.state.holdsSeat).length;
  const recipients = rows.filter((r) => r.state.holdsSeat && r.summary.delivering).length;

  // 검색어에 걸린 행. 칩의 숫자는 이 위에서 센다 — 누르면 몇 명이 남는지가 곧 그 숫자다.
  ...
  const counts = useMemo(() => {
    const c: Record<Filter, number> = { all: searched.length, active: 0, none: 0, expired: 0, withdrawn: 0 };
    for (const r of searched) c[r.state.key] += 1;
    return c;
  }, [searched]);

  const bySubscription = useMemo(
    () => (filter === 'all' ? searched : searched.filter((r) => r.state.key === filter)),
    [searched, filter],
  );

  const alertCounts = useMemo(() => {
    const c: Record<AlertFilter, number> = { all: bySubscription.length, on: 0, off: 0, paused: 0 };
    for (const r of bySubscription) c[alertKey(r.summary)] += 1;
    return c;
  }, [bySubscription]);

  const visible = useMemo(() => {
    const list = alertFilter === 'all' ? [...bySubscription] : bySubscription.filter((r) => alertKey(r.summary) === alertFilter);
    list.sort((a, b) => {
    ...
    return list;
  }, [bySubscription, alertFilter, sort]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <FilterRow label="구독" items={FILTERS} counts={counts} value={filter} onChange={setFilter} />
        <FilterRow label="알림" items={ALERT_FILTERS} counts={alertCounts} value={alertFilter} onChange={setAlertFilter} />
      </div>
